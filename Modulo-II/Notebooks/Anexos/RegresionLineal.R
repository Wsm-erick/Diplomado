...
#Bibliotecas ===================================================================
pacman::p_load(ggplot2, tidyverse, lmtest, gridExtra, nortest, broom, plotly)
...
regresion_lineal <- function(formula, data, prop = 0.66, p_value = 0.05){
  ...
  lm_list <- list(lmodel,train_set, test_set, coef_tab, measure_tab, df_rf[,2:4], tests_tab, pred_tab)
  names(lm_list) <- c("modelo","train_set", "test_set", "coefficients", "performance", "values", "tests" , "predict")
...
#Modelo 1 ======================================================================
reg_lin1 <- regresion_lineal(Sepal.Length ~., data = iris)

#Coeficientes
reg_lin1$coefficients

#Rendimiento
reg_lin1$performance
...
predict_df <- data.frame(
  fit = reg_lin1$predict$fit[,1],
  lower = reg_lin1$predict$fit[,2],
  upper = reg_lin1$predict$fit[,3]
)
len_df = length(predict_df$fit)

ggplotly(
ggplot(predict_df, aes(x = 1:len_df, y = fit)) + 
  geom_point(aes(x = 1:len_df, y = fit), color = "navy") + 
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "maroon", alpha = 0.6) +
  geom_line(color = "navy", linewidth = 0.5) +
  labs(x = NULL, y = "Valor ajustado", 
       title = "Predicciones con bandas de confianza") +
  theme_minimal()
)
...
#Modelo 2 ======================================================================
reg_lin2 <- regresion_lineal(Sepal.Length ~. - Petal.Width, data = iris)

#Coeficientes
reg_lin2$coefficients

#Rendimiento
reg_lin2$performance
